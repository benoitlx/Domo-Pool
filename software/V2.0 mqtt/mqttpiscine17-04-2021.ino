...
#include <simpleMinuteur.h> // https://forum.arduino.cc/index.php?topic=375533.0
// correction de la mesure du pH et du redox
...
OneWire oneWire(2); //PIN digital de la sonde sur la pin 7 de l'arduino
...
float get_temp(int numAdress){
  ...
  float t = sensors.getTempCByIndex(numAdress);
...
float pH(float ph_offset){
  float ph_sensor_value = analogRead(A1) * 5 / 1024;     //convert 0.0-5v
  float ph_value_float = 3.5 * ph_sensor_value + ph_offset;   //pour calculer le ph
...
{
  float redox_sensor_value = analogRead(A0) * 5 / 1024;   // form 0.0 to 5.0 V
  float redox_value_float = ((2.5 - redox_sensor_value) / 1.037) * 1000.0;
...
float pressure(){
  ...
  float pressureB = (3.0*((float)vPressure) - 0.5)*10;
  return pressureB/10;
}
